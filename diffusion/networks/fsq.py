...
class FSQ(Module):
    ...
    def __init__(
        ...
        keep_num_codebooks_dim = default(keep_num_codebooks_dim, num_codebooks
                                         > 1)
        ...
        if self.training:
            ...
            quantize_mask = torch.bernoulli(
                torch.full([z.shape[0], 1, 1, 1],
                           self.noise_dropout,
                           device=z.device)).bool().expand_as(z)

            if preserve_symmetry:
                quantized = round_ste(
                    self.symmetry_preserving_bound(z)) / half_width
            else:
            ...
            quantized = torch.where(offset_mask, unquantized + offset,
                                    quantized)
    ...
    def forward(self, z):
        ...
        quantization_context = partial(
            autocast, 'cuda', enabled=False) if force_f32 else nullcontext
